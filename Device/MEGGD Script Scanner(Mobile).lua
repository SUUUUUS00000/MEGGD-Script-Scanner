...
getgenv().api_decompile_shiny = function(scr)
    ...
    local res = http_request({
        Url = "https://decompile-6o2g.onrender.com/luau/decompile",
        Method = "POST",
        Body = base64_encoder(bytecode)
    })

if not res then
    return "-- api request error: NO RESPONSE (check if URL is correct or server is down)"
...
local function make_scrollbar_interactive(scroll_frame)
    ...
    scroll_frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            if input.Position.X >= rect.X + size.X - thickness - 10 and input.Position.X <= rect.X + size.X + 5 then
                is_dragging = true
